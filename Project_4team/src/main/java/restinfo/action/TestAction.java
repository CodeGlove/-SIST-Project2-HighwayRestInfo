...
public class TestAction implements Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		if ("POST".equals(request.getMethod())) {
			...
			// "searchRestAreas" 액션인 경우 휴게소 검색 API 호출
			else if ("searchRestAreas".equals(action)) {
				System.out.println("휴게소 검색 API 호출 시작");
				try {
					// 클라이언트로부터 전송된 중심점 좌표 파싱
					String centerX = request.getParameter("centerX"); // 중심점 경도
					String centerY = request.getParameter("centerY"); // 중심점 위도
					
					System.out.println("검색 중심점: " + centerX + ", " + centerY);
					
					// NAVER Cloud Platform Place API URL 구성
					// API 엔드포인트: https://maps.apigw.ntruss.com/map-place/v1/search
					String placeUrl = "https://maps.apigw.ntruss.com/map-place/v1/search";
					
					// 쿼리 파라미터 구성: query=휴게소&coordinate=경도,위도&radius=5000&display=10
					// query: 검색 키워드, coordinate: 중심점 좌표, radius: 검색 반경(미터), display: 결과 개수
					String queryParams = "query=휴게소&coordinate=" + centerX + "," + centerY + "&radius=10000&display=10";
					String fullUrl = placeUrl + "?" + queryParams;
					...
					System.out.println("=== Place API 인증 정보 ===");
					System.out.println("Access Key ID: " + accessKeyId);
					System.out.println("Access Key: " + accessKey);
					...
					br.close();
					connection.disconnect(); // HTTP 연결 명시적 종료
					...
					response.setHeader("Access-Control-Allow-Headers", "Content-Type");
					response.setHeader("Content-Length",
							String.valueOf(responseText.getBytes(StandardCharsets.UTF_8).length));
					
					// 응답 데이터를 클라이언트로 전송
					response.getWriter().print(responseText);
					response.getWriter().flush();
					response.getWriter().close();
					System.out.println("휴게소 검색 JSON 응답 전송 완료");
					return null; // 더 이상 JSP 처리하지 않음
					
				} catch (Exception e) {
					// 예외 발생 시 오류 정보를 JSON 형태로 클라이언트에 전송
					System.out.println("휴게소 검색 API 오류: " + e.getMessage());
					e.printStackTrace();
					response.setContentType("application/json");
					response.setCharacterEncoding("UTF-8");
					response.setHeader("Access-Control-Allow-Origin", "*");
					...
					try {
						...
						System.out.println("응답 작성 중 오류: " + ioException.getMessage());
					}
					return null; // 더 이상 처리하지 않음
				}
			} else {
		...
		// POST 요청이 아니거나 directions 액션이 아닌 경우 navermap.jsp 페이지 반환
		return "navermap.jsp";
	}
}
